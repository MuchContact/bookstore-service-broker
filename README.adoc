...
== Getting Started
如果想看到内存数据库数据请执行以下代码，并将datasource.url注释行打开（hsqldb-2.4.1.jar包在该项目依赖里，可以去本地jar仓库找）：

    java -cp hsqldb-2.4.1.jar org.hsqldb.server.Server --database.0 file.testdb --dbname.0 broker-db
